...
class_name ScanLinesArtifactEffect
...
# Exposed parameters
@export var frequency: float = 10.0       # Frequency for sine wave modulation
@export var line_brightness: float = 0.5 # Brightness of scan lines (0 = dark, 1 = light)
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	# Push constants: screen size, sine wave frequency, and line brightness
	var push_constants: PackedFloat32Array = PackedFloat32Array()
	push_constants.append(size.x)           # screen_size.x
	push_constants.append(size.y)           # screen_size.y
	push_constants.append(frequency)        # Frequency of sine wave
	push_constants.append(line_brightness)  # Brightness of scan lines
...
func initialize_computer_shader() -> void:
	...
	var glsl_file: RDShaderFile = load("res://Assets/Compositor Effects/ScreenLineScan.glsl")
