...
class_name ScreenWarping

var rd: RenderingDevice
var shader: RID
var pipeline: RID

# Curvature variable (exposed to editor)
@export_range(-100.0,100.0) var curvature : float = 1.0
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	# Push constants: screen size and curvature
	...
	push_constants.append(curvature)       # curvature value
...
func initialize_computer_shader() -> void:
	...
	var glsl_file: RDShaderFile = load("res://Assets/Compositor Effects/ScreenWarping.glsl")
