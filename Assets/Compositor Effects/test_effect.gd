...
func _init() -> void:
	...
	var parmater_data := PackedFloat32Array([0.0, 0.0, 0.0, 0.0]).to_byte_array()
	parameter_storage_buffer = rd.storage_buffer_create(parmater_data.size(), parmater_data)
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	var parmater_data := PackedFloat32Array([size.x, size.y, 0.0, 0.0]).to_byte_array()
	rd.buffer_update(parameter_storage_buffer, 0, parmater_data.size(), parmater_data)
