...
# Access the rendering device
var rd := RenderingServer.get_rendering_device()

# RIDs for shader, pipeline, and buffer
var shader := RID()
var pipeline := RID()
...
func _init() -> void:
	...
	var shader_file: RDShaderFile = load("res://Assets/Compositor Effects/test_shader.glsl")
	var shader_spirv = shader_file.get_spirv()
	shader = rd.shader_create_from_spirv(shader_spirv)
	pipeline = rd.compute_pipeline_create(shader)

	# Create a storage buffer for shader parameters
...
func _render_callback(_callback_type: int, render_data: RenderData) -> void:
	...
	var groups: Vector3i = Vector3(
		(size.x - 1.0) / 8.0 + 1.0,
		(size.y - 1.0) / 8.0 + 1.0,
		1.0
	).floor()

	# Update buffer with new screen size
	...
	# Bind uniforms and dispatch compute
	var bindings = [
		parameter_uniform,
		color_layer_uniform
	]
